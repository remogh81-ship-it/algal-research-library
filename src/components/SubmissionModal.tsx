...
export function SubmissionModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="modal-backdrop" onClick={onClose}><section className="modal submission-modal" onClick={(event) => event.stopPropagation()}>
    ...
    <form onSubmit={submit} className="submission-form"><div className="form-grid"><label>English title<input value={values.title} onChange={(event) => update('title', event.target.value)} required /></label><label>Arabic title<input value={values.titleArabic} onChange={(event) => update('titleArabic', event.target.value)} required /></label><label>Authors<input value={values.authors} onChange={(event) => update('authors', event.target.value)} required /></label><label>Publication year<input type="number" value={values.year} onChange={(event) => update('year', event.target.value)} required /></label><label>Category<select value={values.category} onChange={(event) => update('category', event.target.value)}><option>Microalgae</option><option>Macroalgae</option><option>Biofuel</option><option>Bioremediation</option><option>Wastewater Treatment</option><option>Carbon Capture</option></select></label><label>Algae type<select value={values.algaeType} onChange={(event) => update('algaeType', event.target.value)} required><option value="microalgae">Microalgae</option><option value="macroalgae">Macroalgae</option></select></label><label>Journal name<input value={values.journal} onChange={(event) => update('journal', event.target.value)} required /></label><label>DOI<input value={values.doi} onChange={(event) => update('doi', event.target.value)} /></label><label>Landing page URL<input type="url" value={values.url} onChange={(event) => update('url', event.target.value)} /></label><label>PDF URL<input type="url" value={values.pdfUrl} onChange={(event) => update('pdfUrl', event.target.value)} /></label></div><label>Arabic summary<textarea value={values.summary_ar} onChange={(event) => update('summary_ar', event.target.value)} required /></label>{error && <p className="error">{error}</p>}<button className="primary-action" type="submit">Save research</button></form>
  </section></div>;
}
