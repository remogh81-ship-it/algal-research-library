...
import { BarChart3, Bot, Copy, Download, FileDown, FileText, GitCompare, Send, Settings, Search, Tags, X } from 'lucide-react';
...
export function AiChatWidget() {
  ...
  const recognized = useMemo(() => {
    const terms = ['Chlorella', 'Spirulina', 'Scenedesmus', 'Nannochloropsis', 'Biofuel', 'Heavy Metal Biosorption', 'CO2 Fixation'];
    return terms.filter((term) => query.toLocaleLowerCase().includes(term.toLocaleLowerCase()) || answer.toLocaleLowerCase().includes(term.toLocaleLowerCase()));
  }, [answer, query]);
  const copyAnswer = async () => { if (answer) { await navigator.clipboard.writeText(answer); setToast('Copied to clipboard'); window.setTimeout(() => setToast(''), 1800); } };
  ...
  const exportSelected = (format: 'apa' | 'mla' | 'bibtex') => {
    ...
    if (citations) { void navigator.clipboard.writeText(citations); setToast('Citation copied to clipboard!'); window.setTimeout(() => setToast(''), 1800); }
  };
  const copyResourceCitation = async (resource: AssistantResult['resource']) => {
    await navigator.clipboard.writeText(formatAPA(resource));
    setToast('Citation copied to clipboard!');
