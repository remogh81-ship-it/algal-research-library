...
export default function GrowthRateCalculator() {
  ...
  return (
    <div
      className="w-full max-w-4xl mx-auto p-6 bg-slate-900 text-slate-100 rounded-2xl shadow-xl border border-slate-800"
      dir={isRtl ? 'rtl' : 'ltr'}
    >
      <div className="flex flex-col md:flex-row md:items-center justify-between pb-6 border-b border-slate-800 gap-4">
        <div>
          <h2 className="text-2xl font-bold text-emerald-400 flex items-center gap-2">
            <span>🧪</span> {t('growthCalculator.title')}{' '}
            <span className="text-sm font-normal text-slate-400">({t('growthCalculator.subtitle')})</span>
          </h2>
          <p className="text-slate-400 text-sm mt-1">
            {t('growthCalculator.description')}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-6">
        <div className="lg:col-span-5 space-y-4">
          <div className="flex justify-between items-center">
            <h3 className="text-sm font-semibold text-slate-300">{t('growthCalculator.tableTitle')}</h3>
            <button
              type="button"
              onClick={addPoint}
            ...
            >
              <span>+</span> {t('growthCalculator.addPoint')}
            </button>
          </div>

          <div className="overflow-x-auto bg-slate-950/60 rounded-xl border border-slate-800">
            <table className={`w-full text-xs text-slate-300 ${isRtl ? 'text-right' : 'text-left'}`}>
              <thead className="bg-slate-800/80 text-slate-400 border-b border-slate-800">
                <tr>
                  ...
                  <th className="p-2.5">{t('growthCalculator.colOd')}</th>
                  <th className="p-2.5 text-center">{t('growthCalculator.colDelete')}</th>
                </tr>
              </thead>
                  ...
                  <tr key={point.id} className="hover:bg-slate-800/30">
                    <td className="p-2">
                      <input
                        ...
                        className="w-full bg-slate-900 border border-slate-700 rounded px-2 py-1 text-slate-200 focus:border-emerald-500 outline-none ltr"
                        dir="ltr"
                      />
                    </td>
                    <td className="p-2">
          ...
          <div className="p-4 bg-slate-800/40 rounded-xl border border-slate-800 space-y-3">
            <h4 className="text-xs font-bold text-slate-300">{t('growthCalculator.phaseSelection')}</h4>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[11px] text-slate-400 mb-1">{t('growthCalculator.startPhase')}</label>
                <select
                  value={startPointId}
              ...
              <div>
                <label className="block text-[11px] text-slate-400 mb-1">{t('growthCalculator.endPhase')}</label>
                <select
                  value={endPointId}
        ...
        <div className="lg:col-span-7 flex flex-col justify-between space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
              <span className="text-xs text-emerald-400 font-medium block">{t('growthCalculator.specificRate')}</span>
              <div className="text-3xl font-extrabold text-emerald-400 mt-1" dir="ltr">
            ...
            <div className="p-4 bg-cyan-500/10 border border-cyan-500/20 rounded-xl">
              <span className="text-xs text-cyan-400 font-medium block">{t('growthCalculator.doublingTime')}</span>
              <div className="text-3xl font-extrabold text-cyan-400 mt-1" dir="ltr">
          ...
          <div className="bg-slate-950/80 p-4 rounded-xl border border-slate-800 relative">
            <h4 className="text-xs font-semibold text-slate-400 mb-2">{t('growthCalculator.chartTitle')}</h4>
            <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="w-full h-auto overflow-visible">
              ...
              {[0.25, 0.5, 0.75].map((ratio, idx) => {
                ...
                return (
                  <line key={`grid-${idx}`} x1={padding} y1={y} x2={chartWidth - padding} y2={y} stroke="#1e293b" strokeDasharray="3 3" />
                );
              })}
